...
export interface CallAudio {
  /** The call currently loaded, or null when nothing is playing. */
  id: string | null
  status: AudioStatus
  error: Error | null
  toggle: (callId: string) => Promise<void>
  stop: () => void
  /** Seconds elapsed, and the track length once known. */
  at: number
  of: number
}
...
function release(audio: HTMLAudioElement | null) {
  if (!audio) return
  audio.onended = null
...
export function useCallAudio(): CallAudio {
  ...
  const [state, setState] = useState<{ id: string | null; status: AudioStatus; error: Error | null }>({
    id: null,
    status: 'idle',
    error: null,
  })
      ...
      if (state.id === callId && audio) {
        ...
        if (state.status === 'paused') {
          try {
            ...
            setState({ id: callId, status: 'error', error: err as Error })
          }
          return
        }
      }
