...
export default function Badge({ tone = 'muted', size = 'md', className, children }: BadgeProps) {
  return (
    <span
      className={cn(
...
export function toneFor(label?: string): string {
  return (label ? TONE_BY_LABEL[label] : undefined) ?? 'muted'
}
...
export function StatusBadge({
  ...
  return (
    <Badge tone={toneFor(label) as BadgeProps['tone']} size={size} className={className}>
      {label}
    </Badge>
  )
}
