...
export function barPathVertical(x, y, w, h, r = 4) {
  const rr = Math.min(r, w / 2, h)
  if (h <= 0) return ''
  return `M${x},${y + h} L${x},${y + rr} Q${x},${y} ${x + rr},${y} L${x + w - rr},${y} Q${x + w},${y} ${x + w},${y + rr} L${x + w},${y + h} Z`
}

export function barPathHorizontal(x, y, w, h, r = 4) {
...
export function Tooltip({ x, y, title, value, containerWidth }) {
...
export function ChartFrame({ title, subtitle, action, children, className }) {
  return (
    <section
      className={cn(
