...
import type { LogRow, LogScore, LogSentiment } from '../lib/types'
...
const MOVERS: Record<string, { label: string; icon: React.ComponentType<IconProps> }> = {
...
const moverOf = (d: string) => MOVERS[d] ?? { label: 'Unknown', icon: IconSystem }

const SENTIMENT_TONE: Record<LogSentiment, string> = {
  Positive: 'text-ok',
  Negative: 'text-danger',
  Neutral: 'text-ink-3',
}
const MOOD: Record<LogSentiment, 'positive' | 'neutral' | 'negative'> = {
  Positive: 'positive',
  Negative: 'negative',
  Neutral: 'neutral',
}
...
function ExportMenu({ row }: { row: LogRow }) {
...
function AiStatus({ ai }: { ai: LogScore | null }) {
 ...
 */
export default function ConversationLog({ rows }: { rows: readonly LogRow[] }) {
