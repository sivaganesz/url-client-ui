/**
 * Client-review backend — shared logic.
 *
 * Transport-agnostic on purpose: server/index.js wraps it for local dev and
 * api/[...path].js wraps it for Vercel. The sanitising rules below are the
 * security boundary, so they live in ONE place — duplicating them per
 * transport is how a leak eventually gets introduced.
 *
...
/** Optional gate. Unset means open; set means callers must supply the code. */
const ACCESS_CODE = process.env.ACCESS_CODE ?? ''
export const gated = ACCESS_CODE.length > 0

/**
 * Route a request. Returns { status, body } — no transport concerns.
 * `pathname` is the full path, e.g. "/api/overview".
 */
export async function handle({ method = 'GET', pathname, code = '' }) {
  if (method !== 'GET') return { status: 405, body: { error: 'This service is read-only.' } }

  // Health stays open so the app can tell "locked" from "broken".
  if (pathname === '/api/health') {
    return { status: 200, body: { ok: true, configured: KEY.length > 0, gated } }
  }

  if (gated && code !== ACCESS_CODE) {
    return { status: 401, body: { error: 'This report is private. An access code is required.' } }
  }

  if (!KEY) {
    return { status: 503, body: { error: 'Not configured. PERFOX_API_KEY is missing.' } }
  }

  try {
    if (pathname === '/api/overview') return { status: 200, body: await overview() }
    if (pathname === '/api/channels') return { status: 200, body: await channels() }
    if (pathname === '/api/customers') return { status: 200, body: await customers() }
    if (pathname === '/api/assistants') return { status: 200, body: await assistants() }
    ...
    const detail = pathname.match(/^\/api\/conversations\/([^/]+)$/)
    if (detail) {
      const id = decodeURIComponent(detail[1])
      if (!UUID.test(id)) return { status: 400, body: { error: 'Not a valid conversation reference.' } }
      const { conversations } = await loadCore()
      ...
      return { status: 200, body: { conversation, transcript: shapeTranscript(rows(events)) } }
    }
    ...
    return { status: 502, body: { error: 'Could not load this right now. Please try again.' } }
  }
}

export const isConfigured = () => KEY.length > 0
